...
class etsis_Mysqldump
{
    ...
    /**
     * Constructor of etsis_Mysqldump. Note that in the case of an SQLite database
    ...
    $dsn = '', $user = '', $pass = '', $dumpSettings = [], $pdoSettings = []
    )
    {
        $dumpSettingsDefault = [
            'include-tables' => [],
            'exclude-tables' => [],
            'compress' => etsis_Mysqldump::NONE,
            'init_commands' => [],
            'no-data' => [],
            ...
            'databases' => false,
            'default-character-set' => etsis_Mysqldump::UTF8,
            'disable-keys' => true,
    ...
    /**
     * Destructor of etsis_Mysqldump. Unsets dbHandlers and database objects.
     *
     */
            ...
            switch ($this->dbType) {
                ...
                case 'dblib':
                    $this->dbHandler = @new PDO(
                        $this->dsn, $this->user, $this->pass, $this->pdoSettings
                    );
        ...
        if (empty($this->dumpSettings['include-views'])) {
            ...
            foreach ($this->dbHandler->query($this->typeAdapter->show_views($this->dbName)) as $row) {
                if (in_array(current($row), $this->dumpSettings['include-views'], true)) {
                    array_push($this->views, current($row));
                    $elem = array_search(
                        current($row), $this->dumpSettings['include-views']
                    );
    ...
    private function exportTables()
    {
        ...
        foreach ($this->tables as $table) {
            ...
            } else if (true === $this->dumpSettings['no-data'] || $this->matches($table, $this->dumpSettings['no-data'])) {
                continue;
            } else {
                $this->listValues($table);
            }
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public function create_view($row)
    {
        ...
        $triggerStmtReplaced1 = str_replace(
            "CREATE ALGORITHM", "/*!50001 CREATE ALGORITHM", $triggerStmt
        );
        $triggerStmtReplaced2 = str_replace(
            " DEFINER=", " */" . PHP_EOL . "/*!50013 DEFINER=", $triggerStmtReplaced1
        );
        $triggerStmtReplaced3 = str_replace(
            " VIEW ", " */" . PHP_EOL . "/*!50001 VIEW ", $triggerStmtReplaced2
        );
    ...
    public function create_event($row)
    {
        ...
        $eventStmtReplaced = str_replace(
            "CREATE DEFINER", "/*!50106 CREATE*/ /*!50117 DEFINER", $eventStmt
        );
        $eventStmtReplaced = str_replace(
            " EVENT ", "*/ /*!50106 EVENT ", $eventStmtReplaced
        );
