...
class TcpdfFonts
{
    ...
    public static function UTF8ArrToLatin1Arr($unicode)
    {
        ...
        foreach ($unicode as $char) {
            if ($char < 256) {
                $outarr[] = $char;
            } elseif (array_key_exists($char, TcpdfFontData::$uni_utf8tolatin)) {
                // map from UTF-8
                $outarr[] = TcpdfFontData::$uni_utf8tolatin[$char];
            } elseif ($char == 0xFFFD) {
                // skip
            } else {
                $outarr[] = 63; // '?' character
    ...
    public static function UTF8ArrToLatin1($unicode)
    {
        ...
        foreach ($unicode as $char) {
            if ($char < 256) {
                ...
                // map from UTF-8
                $outstr .= chr(TcpdfFontData::$uni_utf8tolatin[$char]);
            } elseif ($char == 0xFFFD) {
                // skip
            } else {
                $outstr .= '?';
